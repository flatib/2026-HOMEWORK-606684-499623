...
public class LabirintoBuilderTest {
	...
	public void testBilocale() {
		...
		assertEquals(bilocale.getStanzaVincente(),
				bilocale.getStanzaIniziale().getStanzaAdiacente(Direzione.NORD));
		assertEquals(Collections.singleton(Direzione.NORD),
				bilocale.getStanzaIniziale().getDirezioni());
		assertEquals(Collections.singleton(Direzione.SUD),
				bilocale.getStanzaVincente().getDirezioni());
	}
	...
	public void testTrilocale() {
		...
		assertEquals("biblioteca",
				trilocale.getStanzaIniziale().getStanzaAdiacente(Direzione.SUD).getNome());
	}
	...
	public void testPiuDiQuattroAdiacenti() {
		...
		Map<Direzione, Stanza> mappa = new EnumMap<>(Direzione.class);
	...
	public void testAggiuntaAttrezzoAStanze_MoltepliciAttrezziStessaStanza() {
		...
		assertEquals(new Attrezzo(nomeAttrezzo2, peso2),
				listaStanze.get(nomeStanza1).getAttrezzo(nomeAttrezzo2));
		assertEquals(new Attrezzo(nomeAttrezzo1, peso1),
				listaStanze.get(nomeStanza1).getAttrezzo(nomeAttrezzo1));
	}
	...
	public void testAggiuntaAttrezzoAStanze_AttrezzoAggiuntoAllaSecondaStanza() {
		...
		Map<String, Stanza> listaStanze = labirintoBuilder.getListaStanze();
		assertEquals(new Attrezzo(nomeAttrezzo1, peso1),
				listaStanze.get(nomeStanza2).getAttrezzo(nomeAttrezzo1));
		assertEquals(new Attrezzo(nomeAttrezzo2, peso2),
				listaStanze.get(nomeStanza2).getAttrezzo(nomeAttrezzo2));
	}
	...
	public void testLabirintoConStanzaMagica() {
		...
		StanzaMagica stanzaMagica =
				(StanzaMagica) labirintoBuilder.getListaStanze().get(nomeStanzaMagica);
	...
	public void testLabirintoConStanzaBloccata_ConPassepartout() {
		...
		assertEquals(labirintoBuilder.getListaStanze().get(nomeStanzaVincente),
				labirintoBuilder.getListaStanze().get("stanza bloccata")
	...
	public void testLabirintoConStanzaBloccata_SenzaPassepartout() {
		...
		assertEquals(labirintoBuilder.getListaStanze().get("stanza bloccata"),
				labirintoBuilder.getListaStanze().get("stanza bloccata")
						.getStanzaAdiacente(Direzione.NORD));
	}
	...
	public void testLabirintoCompletoConTutteLeStanze() {
		...
		assertTrue(corridoio.getDirezioni().containsAll(
				Arrays.asList(Direzione.NORD, Direzione.SUD, Direzione.EST, Direzione.OVEST)));

		assertEquals(labirintoCompleto.getStanze().get("corridoio").getMapStanzeAdiacenti(),
				corridoio.getMapStanzeAdiacenti());
